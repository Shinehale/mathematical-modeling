...
%         0.8,-0.7,0.5,1,0.7
%        -0.5,0.2,-0.3,0.7,1];
%   matrixplot(x,'FigShap','s','FigSize','0.8','ColorBar','on');
%   matrixplot(x);
...
scale = str2double(FigSize);
if scale < 0 || scale > 1 || isempty(scale) || isnan(scale)
    scale = 1;
end
...
if ~(any(strncmpi(FigSize,{'Full','Auto'},3)) || (~isempty(str2double(FigSize)) && ~isnan(str2double(FigSize))))
    error('图形大小参数只能为Full, Auto ,数字字符之一');
end
